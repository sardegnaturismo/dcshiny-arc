...
resource "aws_security_group" "fe_main_alb_sg" {
  name        = "${module.env.environment}-${module.env.project}-fe_main-alb-sg"
  vpc_id      = "${var.vpc_id}"
...
resource "aws_security_group" "cf-ip-range-http" {
  name        = "${module.env.environment}-cf-ip-range-http-sg"
  vpc_id      = "${var.vpc_id}"
...
resource "aws_security_group" "cf-ip-range-https" {
  name        = "${module.env.environment}-cf-ip-range-https-sg"
  vpc_id      = "${var.vpc_id}"
