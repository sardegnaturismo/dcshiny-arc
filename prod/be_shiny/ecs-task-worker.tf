...
resource "aws_ecs_task_definition" "worker" {
  ...
  family     = "${module.env.environment}-${module.env.project}-worker-td"
...
[
  {
    ...
    "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "${module.env.environment}-${module.env.project}-worker",
          "awslogs-region": "${module.env.region}"
        }
    },
    "mountPoints": [
        {
...
resource "aws_cloudwatch_log_group" "worker" {
  name = "${module.env.environment}-${module.env.project}-worker"
